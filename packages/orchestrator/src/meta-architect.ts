...
export class MetaArchitect {
  ...
  constructor(
    ...
    private runSwarm: (prompt: string) => Promise<void>
  ) {
  ...
  async backgroundProactiveScan() {
    if (this.isScanning || !CONFIG.memory.enabled) return;
   ...
   */
  async analyzeRepositoryHealth(): Promise<string[]> {
...
Return a list of specific, actionable improvement tasks.`;
   ...
   */
  async triggerSelfImprovement(focus?: string) {
...
This is an AUTONOMOUS mission. You have full authority to refactor as needed.`;
   ...
   */
  async submitPullRequest(branchName: string, title: string, body: string) {
