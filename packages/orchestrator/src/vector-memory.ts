...
export class VectorMemory {
  ...
  private collectionName = "agent_memory";
  ...
  async init() {
    ...
    try {
      ...
      console.log(`[VectorMemory] Current collections:`, collections.collections.map((c: any) => c.name));
  ...
  async addEntry(text: string, metadata: Record<string, any>): Promise<void> {
  ...
  async search(query: string, limit = 5, filter?: any, crossProject = false) {
    if (!this.geminiKey) return [];
    try {
      ...
      if (!crossProject) {
        ...
        if (!finalFilter.must) finalFilter.must = [];
  ...
  async getRecallContext(query: string, limit = 3): Promise<string> {
...
- SUMMARY: ${(meta?.summary as string)?.slice(0, 300) || "none"}...`;
      }).join("\n\n") + "\n";
  }

  async getOmniContext(query: string, limit = 2): Promise<string> {
...
- TAGS: ${meta?.tags?.join(", ") || "none"}`;
   ...
   */
  async getEnhancedMemoryContext(query: string): Promise<string> {
    ...
    if (!localContext && !omniContext && !swarmExperience) return "";
    return "\n=== SWARM MEMORY CONTEXT ===" + localContext + omniContext + swarmExperience + "===========================\n";
  }

  async getSwarmExperienceContext(query: string, limit = 2): Promise<string> {
...
- AGENT: ${meta?.agentId || "unknown"}`;
   ...
   */
  async pruneOldEntries(maxAgeMs = 7 * 24 * 60 * 60 * 1000): Promise<number> {
    ...
    try {
      ...
      const oldIds = results.points
        .filter(p => (p.payload as any)?.timestamp < cutoff)
        .map(p => p.id);
      if (oldIds.length > 0) {
  ...
  async markAsConsolidated(ids: (string | number)[]) {
  ...
  async consolidate(): Promise<KnowledgeInsight[]> {
    ...
    const textToConsolidate = entries.map(e => `Prompt: ${(e.metadata as any)?.prompt ?? "unknown"}\nOutput: ${e.text}`).join("\n\n---\n\n");
  ...
  "tags": ["tag1", "tag2"]
    ...
    try {
      ...
      if (parsed.title && parsed.content) {
        ...
        await this.markAsConsolidated(entries.map(e => e.id as string));
