...
export class MetaAgent {
  constructor(
    private backends: Map<string, AIBackend>, 
    private defaultBackendId: string,
    private onThought?: (thought: string) => void
  ) {}

  async analyzeAndAssemble(prompt: string, context?: string): Promise<SwarmSpec | null> {
...
- UI/CSS/QA/Analysis: gemini-api (Fast, Cost-Effective)
- Frontend/Backend/Architecture: groq (High-Performance Llama 3.3)
- Deep Reasoning/Complex Logic: deepseek (DeepSeek R1)
- Thai Support: typhoon (if available) or gemini-api
...
5. Data/Research/Docs heavy: Add 'Rachel' (Researcher).
...
Task: ${prompt}${context ? `\n\nTask Context (Memory/Insights):\n${context}` : ""}`;
   ...
   */
  async reevaluateSwarm(missionPrompt: string, status: string, agents: any[]): Promise<SwarmSpec | null> {
...
Return ONLY valid JSON following the same schema as swarm assembly.`;
  ...
  private async callArchitect(backend: AIBackend, prompt: string): Promise<SwarmSpec | null> {
    const availableBackends = Array.from(this.backends.keys());
