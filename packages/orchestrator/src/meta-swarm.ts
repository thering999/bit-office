...
import type { MetaAgent, SwarmSpec } from "./meta-agent.js";
...
export class MetaSwarm extends EventEmitter {
  ...
  async checkHealth() {
    ...
    let score = 1.0;
    if (errorCount > 0) score -= (errorCount / total) * 0.5;
    
    const status = score > 0.8 ? "optimal" : score > 0.4 ? "stressed" : "failing";
    
    const diagnostics: string[] = [];
    if (errorCount > 0) diagnostics.push(`${errorCount} agents are in error state.`);
    if (workingCount === 0 && total > 0) diagnostics.push("Swarm is idle but mission is not complete.");
