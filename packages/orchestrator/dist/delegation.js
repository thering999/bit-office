...
export class DelegationRouter {
    ...
    worktreeEnabled;
    worktreeMerge;
    constructor(agentManager, promptEngine, emitEvent, worktreeEnabled = false, worktreeMerge = true) {
    ...
    resultTaskDidNotDelegate(taskId) {
        const meta = this.tasks.get(taskId);
        if (!meta?.isResultTask || meta.delegationsAtStart === undefined)
            return false;
    ...
    hasPendingFrom(agentId) {
        for (const meta of this.tasks.values()) {
            if (meta.origin === agentId && !meta.isResultTask)
                return true;
        }
        return false;
    }
    ...
    setTeamProjectDir(dir) {
        ...
        if (dir)
            console.log(`[Delegation] Team project dir set: ${dir}`);
    }
    getTeamProjectDir() {
    ...
    wireDelegation(session) {
        session.onDelegation = (fromAgentId, targetName, prompt) => {
            ...
            let repoPath = this.teamProjectDir ?? undefined;
            ...
            if (this.worktreeEnabled && repoPath && !target.worktreePath && !target.hasSessionHistory) {
                ...
                const activeDevs = this.agentManager.getAll().filter(a => a.agentId !== target.agentId && a.teamId === target.teamId
    ...
    wireResultForwarding(session) {
        session.onTaskComplete = (agentId, taskId, summary, success, fullOutput) => {
            if (this.stopped)
                return;
            const meta = this.tasks.get(taskId);
            if (!meta || meta.isResultTask)
                return; // Result tasks are not forwarded — they're leader-internal
            ...
            if (!fromRole.includes("review") && fullOutput) {
                ...
                if (em)
                    lines.push(`ENTRY_FILE: ${em[1].trim()}`);
                if (cm)
                    lines.push(`PREVIEW_CMD: ${cm[1].trim()}`);
                if (pm)
                    lines.push(`PREVIEW_PORT: ${pm[1]}`);
                if (lines.length > 0)
                    this.lastDevPreview = lines.join("\n");
            }
    ...
    flushResults(originAgentId) {
        ...
        if (this.leaderRounds > CONFIG.delegation.hardCeilingRounds) {
            ...
            const resultLines = pending.results.map(r => `- ${r.fromName} (${r.statusWord}): ${r.summary}`).join("\n");
            this.emitEvent({
                type: "team:chat",
