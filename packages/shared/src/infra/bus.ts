...
export class SwarmBus extends EventEmitter {
  private static instance: SwarmBus;
  private logPath?: string;
  private isNode: boolean;
  private anyListeners: Array<(event: any) => void> = [];
  ...
  public static getInstance(): SwarmBus {
  ...
  private flushLogs() {
    if (!this.isNode || !this.logPath || this.logBuffer.length === 0) return;
   ...
   */
  public onAny(listener: (event: any) => void) {
   ...
   */
  public async emitEvent(event: OrchestratorEvent) {
